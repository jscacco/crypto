...
def score_string(input):
    ...
    for c in input.lower():
        if c in ALPHABET:
            total_let += 1
        elif ord(c) not in range(128):
            return 999999

    # If the string is empty, it can't be what we want
    if total_let == 0:
            return 999999
...
def super_decrypt(messages):
    """Takes an array of strings and returns the decrypted version of the encrypted line."""
    ...
    for message in messages:
        answer = decrypt(message)
        score = score_string(answer[1])
        if(score < best_score):
            best_score = score
            best_string = answer[1]
            key_char = answer[0]
